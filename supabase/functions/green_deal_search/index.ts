...
const pinecone_namespace_green_deal =
  Deno.env.get("PINECONE_NAMESPACE_GREEN_DEAL") ?? "";

const opensearch_region = Deno.env.get("OPENSEARCH_REGION") ?? "";
const opensearch_domain = Deno.env.get("OPENSEARCH_DOMAIN") ?? "";
const opensearch_index_name =
  Deno.env.get("OPENSEARCH_GREEN_DEAL_INDEX_NAME") ?? "";
...
const search = async (
  ...
  const docList = combinedDocs.map((doc) => {
    ...
    const source_entry =
      `${issue_agency}: **${title}(${document_id})**. ${publish_date}`;
